...
# Define the directories where the parser files are located to shorten the rules later in the makefile
PARSER_SRC_DIR=$(SRCDIR)/shell/parser
PARSER_INC_DIR=$(INCDIR)/shell/parser

# Add the C++ source files that are generated by bison and flex. They need to be added to the list of source files
# manually since the PROS build system scans the source directories for source files before they are created
CXXSRC:=$(PARSER_SRC_DIR)/parser.y.cpp
CXXSRC+=$(PARSER_SRC_DIR)/lexer.l.cpp

# Generate the lexer
$(PARSER_SRC_DIR)/lexer.l.cpp: $(PARSER_SRC_DIR)/lexer.l $(PARSER_SRC_DIR)/parser.y.cpp
	$(FLEX) -o $@  $(FLEX_FLAGS) $<

# Generate the yacc parser
$(PARSER_SRC_DIR)/parser.y.cpp: $(PARSER_SRC_DIR)/parser.y
	$(BISON) -o $@ --defines=$(PARSER_INC_DIR)/parser.y.hpp $(BISON_FLAGS) $<

# Override PROS's clean rule so that we can remove the stale bison/flex files
clean_all: clean
	rm -f $(PARSER_SRC_DIR)/parser.y.cpp
	rm -f $(PARSER_SRC_DIR)/lexer.l.cpp
	rm -f $(PARSER_INC_DIR)/parser.y.hpp
	rm -f $(PARSER_INC_DIR)/lexer.l.hpp
